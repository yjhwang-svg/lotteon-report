...
def main():
    ...
    st.markdown(
        "아래 두 파일을 업로드한 뒤 **변환하기**를 누르면 **작업완료 롱포맷**과 "
        "**리포트 업로드용 피벗** 두 종류의 `.xlsx`를 받을 수 있습니다."
    )
    ...
    with st.expander("채널 인덱스 시트 자동 참조 · 권한 설정"):
        st.markdown(
            f"리포트 업로드용 피벗에서 BSA·SA·RT 채널의 채널상세를 비우기 위해, "
            f"아래 스프레드시트 탭을 자동으로 참조합니다. "
            f"스프레드시트 ID: `{REPORT_INDEX_SPREADSHEET_ID}` · gid: `1655143850`  \n"
            "**AI(챗봇)에게 구글 로그인을 줄 수는 없습니다.** 아래 둘 중 하나를 하면 "
    ...
    if st.button("변환하기", type="primary"):
        ...
        with st.spinner("변환 중..."):
            try:
                ...
                ch_names, ch_source = resolve_report_upload_channel_names(st.secrets)
        ...
        if not ch_names:
            st.info(
                ...
                "또는 저장소의 `report_upload_bsa_sa_rt_channels.csv`를 확인해 주세요."
            )
        c1, c2 = st.columns(2)
        with c1:
            st.download_button(
